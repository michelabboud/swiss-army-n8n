...
- All monitor UIs group services by profile (or `(no-profile)`) while preserving compose order; headers remain wrapped.

## [0.1.23] – 2025-11-27

### Fixed

- Prompt UI grouping no longer crashes on group markers when computing column widths.
...
- Prompt and Textual monitors use wrapped, multi-line headers for profiles/services with a separator for readability. Functionality unchanged.
